...
#[path = "src/source_norm.rs"]
mod source_norm;

#[path = "src/source_taxonomy.rs"]
mod source_taxonomy;

use source_taxonomy::source_kind_rust_expr_for_folder;

const OPENCPN_SOURCES_XML: &str = include_str!("data/opencpn_enc_sources.xml");
/// Update when adding/removing catalogs in `data/opencpn_enc_sources.xml`.
...
fn main() {
    ...
    println!("cargo:rerun-if-changed=src/source_norm.rs");
    ...
    fs::write(
        &out_path,
        generate_rust(EXPECTED_SOURCE_COUNT, &sources),
    )
...
fn build_record(name: String, location: String, dir: String) -> Option<SourceRecord> {
    ...
    let kind = source_kind_rust_expr_for_folder(&folder)?;
    Some(SourceRecord {

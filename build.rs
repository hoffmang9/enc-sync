...
#[path = "src/source_folder.rs"]
mod source_folder;

use source_folder::{classify_folder, folder_kind_rust_expr};
...
fn main() {
    ...
    println!("cargo:rerun-if-changed=src/source_folder.rs");
...
fn build_record(name: String, location: String, dir: String) -> Option<SourceRecord> {
    ...
    let kind = classify_folder(&folder).map(|value| folder_kind_rust_expr(&value))?;
    Some(SourceRecord {
